...
namespace pwiz.ProteowizardWrapper.Common.Collections
{
    ...
    public static class CollectionUtil
    {
        ...
        public static bool EqualsDeep<TKey, TValue>(IDictionary<TKey, TValue> dict1, IDictionary<TKey, TValue> dict2)
        {
            ...
            {
                if (!dict2.TryGetValue(entry.Key, out var value2))
                {
                    return false;
                }
        ...
        public static Range BinarySearch<TItem>(IList<TItem> items, Func<TItem, int> compareFunc)
        {
            ...
            foreach (var firstIndex in new[] { true, false })
            {
                var lo = result.Start;
                var hi = result.End;
        ...
        public static Comparer<object> ColumnValueComparer => Comparer<object>.Create(CompareColumnValues);
